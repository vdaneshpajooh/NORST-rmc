...
%             PP1 = P(:, 1 : r_0 + r_1) * rot_matrix1 * Irr(:, 1 : r_0);
	    %%SS2
            ...
            L(:, 1:t_max) = P(:, 1:r_0) * coeff_train(:, 1:t_max);
            %%SS2
	    %             L(:, t_1+1:t_2) = PP1 * coeff_train(:, t_1+1:t_2);
%             L(:, t_1 + 1 : end) = PP1 * coeff_train(:, t_1+1:end);
            
            rho = 0.000; %denotes fraction of number of missing entries
            BernMat = rand(n, t_max);
            T_miss = 1 .* (BernMat <= 1- rho);
%             T_miss(T_miss==0) = 1e4;
...
%                 ev_thresh, alpha, K, omega, s, 5);
            [x_cs_hat, L_hat, P_hat, S_hat, T_hat, t_hat, ...
            P_track_full, t_calc] = ...
            NORST(M(:, t_train + 1 :end),T_miss,...
...
%                 end
        ...
        cnt = cnt + 1;
% t_idx = find(T_hat==255);
% t_miss_idx = find(T_miss(:,t_train+1:end)==10000);
% a = length(intersect(t_idx,t_miss_idx))/length(t_miss_idx)
