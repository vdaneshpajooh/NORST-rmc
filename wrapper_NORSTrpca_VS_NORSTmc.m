...
%% Compute performance metrics
        ...
        for jj = 1 : length(t_calc_rpca)
            ...
            elseif((t_calc_rpca(jj) >= t_1) && (t_calc_rpca(jj) < t_2))
                if(NORSTR == 1)
                tempRPCA_SE_Phat_P(mc, jj) = ...
...
end_time = clock;
save('RMC_NORST_changingSS_MC100_rhomiss10_rhosparse5_ttrain400_WITHmissentries.mat');
% 
% str1 = 't';
% str2 = '$$\log SE(\hat{P}, P)$$';
...
% figure
% 
% plot(t_calc_rpca + t_train,log10(SE_Phat_P_rpca),'*k--','LineWidth',1,'MarkerSize',6)
% hold on
% plot(t_calc_rpca + t_train,log10(SE_Phat_P_GRASTA),'b^--','LineWidth',1,'MarkerSize',6)
% plot(t_calc_rpca + t_train,log10(SE_Phat_P_NCRMC),'ys--','LineWidth',1,'MarkerSize',6)
% grid on
